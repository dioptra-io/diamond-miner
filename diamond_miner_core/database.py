...
def build_next_round_query(
    ...
    query = (
        ...
        f" arrayMap(t->t.2, links_per_ttl) as n_links_per_ttl_no_ttl, "
        f" arrayMap(t->(t, arrayUniq(arrayFilter(x->x.1.2==t, links_flat_previous))), ttls) as links_per_ttl_previous, "
        f" arrayMap(t->t.2, links_per_ttl_previous) as n_links_per_ttl_no_ttl_previous, "
        ...
        f"arrayFilter(x->x.1.3 < {round_number} and x.2.3 < {round_number}, links) as links_previous, "
        " arrayMap(x->((x.1.1, x.1.2), (x.2.1, x.2.2)), links) as links_no_round, "
        " arrayMap(x->((x.1.1, x.1.2), (x.2.1, x.2.2)), links_previous) as links_no_round_previous, "
        "max(round) as max_round "
        "SELECT  "
        "    src_ip,  "
        "    dst_prefix,  "
